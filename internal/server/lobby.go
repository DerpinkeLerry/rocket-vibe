...
func sanitizeLobbyConfig(input game.Config) game.Config {
	...
	config.Arena.Width = bounded(config.Arena.Width, defaults.Arena.Width, 60, 240)
	config.Arena.Length = bounded(config.Arena.Length, defaults.Arena.Length, 80, 360)
	config.Arena.Ceiling = bounded(config.Arena.Ceiling, defaults.Arena.Ceiling, 10, 80)
	...
	config.Car.HalfExtents.X = bounded(config.Car.HalfExtents.X, defaults.Car.HalfExtents.X, 0.2, 2.5)
	config.Car.HalfExtents.Y = bounded(config.Car.HalfExtents.Y, defaults.Car.HalfExtents.Y, 0.1, 1.5)
	config.Car.HalfExtents.Z = bounded(config.Car.HalfExtents.Z, defaults.Car.HalfExtents.Z, 0.3, 3.5)
	...
	config.Car.SupersonicSpeed = bounded(config.Car.SupersonicSpeed, defaults.Car.SupersonicSpeed, 0, config.Car.MaxBoostSpeed)
	...
	config.Car.AirThrottleAcceleration = bounded(config.Car.AirThrottleAcceleration, defaults.Car.AirThrottleAcceleration, 0, 20)
	config.Car.AirReverseAcceleration = bounded(config.Car.AirReverseAcceleration, defaults.Car.AirReverseAcceleration, 0, 20)
	...
	config.Car.JumpMinimumHoldDuration = bounded(config.Car.JumpMinimumHoldDuration, defaults.Car.JumpMinimumHoldDuration, 0, config.Car.JumpHoldDuration)
	config.Car.JumpStickyAcceleration = bounded(config.Car.JumpStickyAcceleration, defaults.Car.JumpStickyAcceleration, 0, 40)
	config.Car.JumpStickyDuration = bounded(config.Car.JumpStickyDuration, defaults.Car.JumpStickyDuration, 0, 0.25)
	...
	config.Ball.Radius = bounded(config.Ball.Radius, defaults.Ball.Radius, 0.4, 6)
	...
	config.Ball.RestingHeight = bounded(config.Ball.RestingHeight, defaults.Ball.RestingHeight, config.Ball.Radius, config.Ball.Radius+0.5)
	config.Ball.SpawnY = bounded(config.Ball.SpawnY, defaults.Ball.SpawnY, config.Ball.RestingHeight, 20)
