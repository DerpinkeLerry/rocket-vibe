...
func TestStatePacketMatchesBrowserLayout(t *testing.T) {
	snapshot := game.Snapshot{
		Tick: 0x11223344, ConnectedMask: 0x85, GroundMask: 0x41, DemolishedMask: 0x84,
		OrangeScore: 7, BlueScore: 9,
		Boost:        [game.MaxPlayers]uint8{100, 55, 20, 0, 99, 88, 77, 66},
		BoostPadMask: (uint64(1) << 33) | 0xa55a,
	}
	...
	if packet[0] != MessageState || binary.LittleEndian.Uint32(packet[1:5]) != 0x11223344 {
		t.Fatalf("invalid packet header: %v", packet[:8])
	}
	if packet[5] != 0x85 || packet[6] != 0x41 || packet[7] != 0x84 {
		t.Fatalf("invalid masks: %08b %08b %08b", packet[5], packet[6], packet[7])
	}
	if binary.LittleEndian.Uint16(packet[8:10]) != 7 || binary.LittleEndian.Uint16(packet[10:12]) != 9 {
		t.Fatalf("invalid score: %v", packet[8:12])
	}
	if packet[12] != 100 || packet[13] != 55 || packet[18] != 77 || packet[19] != 66 {
		t.Fatalf("invalid boost bytes: %v", packet[12:20])
	}
	if binary.LittleEndian.Uint64(packet[20:28]) != (uint64(1)<<33)|0xa55a {
		t.Fatalf("invalid boost pad mask: %016x", binary.LittleEndian.Uint64(packet[20:28]))
	}
	if value := math.Float32frombits(binary.LittleEndian.Uint32(packet[28:32])); value != 1.25 {
